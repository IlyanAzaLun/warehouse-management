...
class Customer extends User
{
    ...
    public function index()
    {
        ...
        $this->data['customers'] = $this->M_users->user_info_select(
            false,
            'Customer'
        );
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->data = [
                'user_fullname' => htmlspecialchars(
                    $this->input->post('user_fullname', true)
                ),
                'user_address' => htmlspecialchars(
                    $this->input->post('user_address', true)
                ),
                'owner_name' => htmlspecialchars(
                    $this->input->post('owner_name', true)
                ),
                'user_address' => htmlspecialchars(
                    $this->input->post('user_address', true)
                ),
                'village' => htmlspecialchars(
                    $this->input->post('village', true)
                ),
                'sub-district' => htmlspecialchars(
                    $this->input->post('sub-district', true)
                ),
                'district' => htmlspecialchars(
                    $this->input->post('district', true)
                ),
                'province' => htmlspecialchars(
                    $this->input->post('province', true)
                ),
                'zip' => htmlspecialchars($this->input->post('zip', true)),
                'user_contact_phone' => htmlspecialchars(
                    $this->input->post('user_contact_phone', true)
                ),
                'user_contact_email' => htmlspecialchars(
                    $this->input->post('user_contact_email', true)
                ),
                'type_id' => htmlspecialchars(
                    $this->input->post('type_id', true)
                ),
            ...
            try {
                ...
                Flasher::setFlash(
                    ...
                    'success',
                    'Success',
                    ' congratulation success to entry data!'
                );
                redirect('customer');
            } catch (Exception $e) {
                Flasher::setFlash(
    ...
    public function import()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!empty($_FILES['file']['name'])) {
                ...
                // array Count
                if (
                    $this->M_customer->customer_insert_multiple(
                        $allDataInSheet
                    ) > 0
                ) {
                    Flasher::setFlash(
