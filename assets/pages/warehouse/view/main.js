		...
		data_customer.user_info(function(output){
			...
			$('input#fullname').on('focusout', function(){
				data_customer.user_info_search($(this).val(), function(data){
					...
					$('input#contact_number').val(`${data.user_contact_phone} (${data.owner_name})`);
					$('textarea#address').val(`${data.user_address}, ${data.village}, ${data['sub-district']}, ${data['district']}, ${data.province}, ${data.zip}`);
				});
